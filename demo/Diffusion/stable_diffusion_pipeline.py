...
class StableDiffusionPipeline:
    ...
    def loadEngines(
        ...
        if int8 or fp8:
            assert self.pipeline_type.is_sd_xl(), "int8 and fp8 quantization only supported for SDXL pipeline"
            use_quant['unetxl'] = True
            quant_str = "int8" if int8 else "fp8"
            print(f"using {quant_str} in loadEngines")
            ...
            if do_export_onnx or do_export_weights_map:
                ...
                if not use_quant[model_name]:
                    ...
                    if not os.path.exists(state_dict_path):
                        ...
                        quant_config = get_quant_config(model, quantization_level, fp8=fp8, int8=int8)
